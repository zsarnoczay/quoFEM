...
DMG_FILENAME=$APP_NAME"_Mac_Download_ARM.dmg"
pathApp=`pwd`/build_arm/$APP_FILE

pathToBackendApps="/Users/fmckenna/NHERI/SimCenterBackendApplications"
pathToOpenSees="/Users/fmckenna/bin/OpenSees3.6.0"
pathToDakota="/Users/fmckenna/dakota/dakota-6.22.0"
...
#
# now conan to build dependencies for UI
#

arch -arm64 conan install .. -pr:b=arm_clang_profile -pr:h=arm_clang_profile --build=missing 

if [[ $0 != 0 ]]; then
    echo "quoFEM: conan install failed";
fi


if [ -n "$release" ] && [ "$release" = "release" ]; then
    echo "******** RELEASE BUILD *************"    
    qmake QMAKE_CXXFLAGS+=-D_SC_RELEASE ../quoFEM.pro QMAKE_APPLE_DEVICE_ARCHS=arm64
    cmd_status=$?; if [[ $cmd_status != 0 ]]; then echo "qmake failed"; exit $cmd_status; fi    
else
    echo "********* NON RELEASE BUILD ********"
    qmake ../quoFEM.pro QMAKE_APPLE_DEVICE_ARCHS=arm64
    cmd_status=$?; if [[ $cmd_status != 0 ]]; then echo "qmake failed"; exit $cmd_status; fi    
fi
...
cp -fr ../../SimCenterBackendApplications/applications_arm/performUQ ./quoFEM.app/Contents/MacOS/applications
cp -fr ../../SimCenterBackendApplications/applications_arm/performFEM ./quoFEM.app/Contents/MacOS/applications
cp -fr ../../SimCenterBackendApplications/applications_arm/Workflow ./quoFEM.app/Contents/MacOS/applications

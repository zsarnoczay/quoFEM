...
#
# env set up
#   NOTE: SDKROOT .. Xcode at 26, does not work with current Qt versions 6.6
#         .. solution download XCode15.4.git, uncompress and copy SDK fles
#         sudo cp -R /Applications/Xcode_15.app/Contents/Developer/Platforms/MacOSX.platform/Developer/SDKs/MacOSX14.5.sdk /Applications/Xcode_15.app/Contents/Developer/Platforms/MacOSX.platform/Developer/SDKs

source /Users/fmckenna/python_env/python3.11-conan/bin/activate
export SDKROOT=/Applications/Xcode.app/Contents/Developer/Platforms/MacOSX.platform/Developer/SDKs/MacOSX14.5.sdk
export PATH=/Users/fmckenna/Qt6/6.10.0/macos/bin:$PATH
...
DMG_FILENAME=$APP_NAME"_Mac_Download_X86.dmg"
pathApp=`pwd`/build_x86/$APP_FILE

pathToBackendApps="/Users/fmckenna/NHERI/SimCenterBackendApplications"
pathToOpenSees="/Users/fmckenna/bin/OpenSees3.6.0"
pathToDakota="/Users/fmckenna/dakota/dakota-6.19.0"


#
# create dir, remove old
#
...
#
# now conan to build dependencies for UI (note not working clang compiler)
#

export CC=/usr/local/bin/gcc-14
export CXX=/usr/local/bin/gcc-14

# Option A: negate the command
if ! arch -x86_64 conan install .. -pr:b=x86_gcc_profile -pr:h=x86_gcc_profile --build=missing;
then
...
#
# now qmake to generate Makefile
#
...
if [ -n "$release" ] && [ "$release" = "release" ]; then
    echo "******** RELEASE BUILD *************"    
    qmake QMAKE_CXXFLAGS+=-D_SC_RELEASE ../quoFEM.pro QMAKE_APPLE_DEVICE_ARCHS=x86_64
    cmd_status=$?; if [[ $cmd_status != 0 ]]; then echo "qmake failed"; exit $cmd_status; fi    
else
    echo "********* NON RELEASE BUILD ********"
    qmake ../quoFEM.pro QMAKE_APPLE_DEVICE_ARCHS=x86_64
    cmd_status=$?; if [[ $cmd_status != 0 ]]; then echo "qmake failed"; exit $cmd_status; fi    
fi
...
cp -fr ../../SimCenterBackendApplications/applications_x86/performUQ ./quoFEM.app/Contents/MacOS/applications
cp -fr ../../SimCenterBackendApplications/applications_x86/performFEM ./quoFEM.app/Contents/MacOS/applications
cp -fr ../../SimCenterBackendApplications/applications_x86/Workflow ./quoFEM.app/Contents/MacOS/applications
